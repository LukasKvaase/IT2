...
for dag in vaerdata["daily"]:
  time = dag["dt"]
  sunrise = dt.datetime.fromtimestamp(dag["sunrise"]).time()
  sunset = dt.datetime.fromtimestamp(dag["sunset"]).time()
  moonrise = dt.datetime.fromtimestamp(dag["moonrise"]).time()
  moonset = dt.datetime.fromtimestamp(dag["moonset"]).time()
  dato = dt.datetime.fromtimestamp(time).strftime("%d %m %Y")
  tempstring = ""
  for time in dag["temp"]:
      tempstring += f"{time} - {(dag["temp"][time]-273.15):5.2f}    "
  
  print(f"Varsel for {dato}. \n{tempstring }\nSoloppgang {sunrise}, Solnedgang {sunset}, Måneoppgang {moonrise}, Månenedgang {moonset}\n")
